...
class Server {
  ...
  setupFeatures() {
    ...
    runnableFeatures.push('setup', 'before', 'coop', 'headers', 'middleware');
  ...
  createServer() {
    ...
    if (this.options.https) {
      ...
      if (semver.gte(process.version, '10.0.0') || !isHttp2) {
        ...
        this.listeningApp = https.createServer(this.options.https, app);
      } else {
        // The relevant issues are:
    ...
    const server = this.listeningApp;
    const connectionCallback = (socket) => {
      // A comprehensive error handling is not possible here, as the underlying TCP socket is not exposed.
      // See https://github.com/nodejs/node/issues/14209 and https://github.com/nodejs/node/issues/20579
      socket.on('error', () => {});
    };

    server.on('connection', connectionCallback);

    // https://github.com/webpack/webpack-dev-server/pull/1974
    // https://github.com/nodejs/node/issues/27387
    server.on('clientError', (err, socket) => {
      if (socket.writable) {
        // a comprehensive error handling is not possible here
        // https://github.com/nodejs/node/issues/14209
        // https://github.com/nodejs/node/issues/20579
        socket.on('error', () => {});
  ...
  createSocketServer() {
    ...
    // sockjs's installHandlers uses overshadowListeners which removes all existing
    // 'request' and 'upgrade' listeners and replaces them with its own.
    // We must re-inject host/origin checking before the sockjs handler runs.
    const requestListeners = this.listeningApp.listeners('request');
    if (requestListeners.length > 0) {
      const sockjsRequestHandler = requestListeners[0];
      this.listeningApp.removeAllListeners('request');
      this.listeningApp.on('request', (req, res) => {
        if (!this.checkHost(req.headers)) {
          res.writeHead(403);
          res.end('Invalid Host header');
          return;
        }
        sockjsRequestHandler(req, res);
      });
    }

    const upgradeListeners = this.listeningApp.listeners('upgrade');
    if (upgradeListeners.length > 0) {
      const sockjsUpgradeHandler = upgradeListeners[0];
      this.listeningApp.removeAllListeners('upgrade');
      this.listeningApp.on('upgrade', (req, socket, head) => {
        if (!this.checkHost(req.headers)) {
          socket.write('HTTP/1.1 403 Forbidden\r\n\r\n');
          socket.destroy();
          return;
        }
        sockjsUpgradeHandler(req, socket, head);
      });
    }
  ...
  isAllowedHost(host) {
    // `host` may be `undefined` when `req.headers.host` is
    if (!host) {
      return false;
    }

    // Guard against missing or non-array allowedHosts
    if (!Array.isArray(this.allowedHosts)) {
      return host === this.publicHost || host === 'localhost' || host === '127.0.0.1';
    }

    return (
      this.allowedHosts.some((allowedHost) => {
        // This allows Cromwell to correctly validate hosts
        // e.g. host: 'example.com', allowedHost: '.example.com'
        if (allowedHost.startsWith('.')) {
          // host ends with allowedHost
  ...
  checkHost(headers) {
    ...
    // We use `url.parse()` to get the hostname from the `Host` header.
    // It handles IP v6 addresses and port numbers like `[::1]:8080`.
    const hostname = host ? url.parse(`http://${host}`).hostname : null;

    // Host header must be valid.
    // Explicit IP addresses (v4/v6) are always valid for the Host header,
    // maintaining backward compatibility with the original checkHeaders logic.
    if (
      !this.isAllowedHost(hostname) &&
      !(hostname && (ip.isV4Format(hostname) || ip.isV6Format(hostname)))
    ) {
      return false;
    }

    // If Origin header is present, it must also be validated
    const origin = headers.origin;

    if (origin) {
      const originHost = url.parse(origin).hostname;

      if (!this.isAllowedHost(originHost)) {
        return false;
      }
    }
